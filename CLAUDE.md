...
The errors `InvalidNodeTypeError: Failed to execute 'selectNode' on 'Range': the given Node has no parent` and `Unchecked runtime.lastError` are generated by **browser extensions** (e.g. Grammarly, LastPass). These are not application bugs and require no action.
